 ...
 * @param {string} config.corsAllowedHeaders - Allowed headers (default: Content-Type,Authorization,Prefer,Range,Resolution,Accept,Accept-Profile,Content-Profile)
...
function createCorsMiddleware(config = {}) {
  ...
  const corsOptions = {
    ...
    // Accept-Profile/Content-Profile are part of the PostgREST protocol
    // (schema selection) — browsers need them allowed for preflight
    allowedHeaders: config.corsAllowedHeaders || 'Content-Type,Authorization,Prefer,Range,Resolution,Accept,Accept-Profile,Content-Profile',

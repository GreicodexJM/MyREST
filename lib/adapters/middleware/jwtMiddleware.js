...
function createJwtMiddleware(config) {
  ...
  return function jwtMiddleware(req, res, next) {
    ...
    if (authHeader && authHeader.split(' ')[0] === 'Bearer') {
      ...
      // Pin to HMAC algorithms: prevents algorithm-confusion attacks
      // (e.g. asymmetric tokens verified against the shared secret)
      jwt.verify(token, config.jwtSecret, { algorithms: ['HS256', 'HS384', 'HS512'] }, (err, decoded) => {
        if (err) {

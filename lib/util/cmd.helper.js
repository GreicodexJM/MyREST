...
program
  ...
  .option('--databases <n>', 'comma-separated list of databases to expose from the same server (multi-database mode; schema selected via PostgREST Accept-Profile/Content-Profile headers or db.table names)')
  .option('--jwtSecret <n>', 'JWT secret for token validation')
  .option('--jwtRequired', 'Require JWT for all requests (default: false)')
  .option('--corsOrigin <n>', 'CORS allowed origins (* for all, or comma-separated list) (default: *)')
  .option('--corsMethods <n>', 'CORS allowed methods (default: GET,POST,PUT,PATCH,DELETE,OPTIONS)')
  .option('--corsAllowedHeaders <n>', 'CORS allowed headers (default: Content-Type,Authorization,Prefer,Range,Resolution,Accept)')
  .option('--corsExposedHeaders <n>', 'CORS exposed headers (default: Content-Range,Location)')
  .option('--corsCredentials', 'CORS allow credentials (default: false)')
  .option('--corsMaxAge <n>', 'CORS preflight cache duration in seconds (default: 3600)')
  .parse(process.argv)
...
exports.handle = program => {
  ...
  /**************** START : multi-database mode ****************/
  if (program.databases) {
    const SYSTEM_SCHEMAS = ['mysql', 'information_schema', 'performance_schema', 'sys'];
    const list = [...new Set(
      String(program.databases).split(',').map(s => s.trim()).filter(Boolean)
    )];

    if (list.length === 0) {
      console.error('--databases was provided but contains no database names'.red);
      process.exit(1);
    }

    const system = list.filter(db => SYSTEM_SCHEMAS.includes(db.toLowerCase()));
    if (system.length > 0) {
      console.error(('System schemas cannot be exposed: ' + system.join(', ')).red);
      process.exit(1);
    }

    // The default schema (--database / DATABASE_URL) must be part of the allowlist
    if (program.database && !list.includes(program.database)) {
      list.unshift(program.database);
    }
    program.databases = list;
    program.database = program.database || list[0];

    console.log(('Multi-database mode: ' + program.databases.join(', ') +
      ' (default: ' + program.database + ')').green);
  } else {
    program.databases = null;
  }
  /**************** END : multi-database mode ****************/
  ...
  program.corsOrigin = program.corsOrigin || '*';
  program.corsMethods = program.corsMethods || '';
  program.corsAllowedHeaders = program.corsAllowedHeaders || '';
  program.corsExposedHeaders = program.corsExposedHeaders || '';
  program.corsCredentials = program.corsCredentials || false;
  program.corsMaxAge = program.corsMaxAge || '';
  ...
  if (program.database && program.host && program.user && program.port) {
    console.log('Starting server at:', 'http://0.0.0.0:' + program.portNumber)
    console.log('Database at:', 'mysql://' + program.user + ':******@' + program.host + ':' + program.port + '/' + program.database + '?ssl=' + program.ssl)
    if (program.ssl) {

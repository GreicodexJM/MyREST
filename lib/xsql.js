...
class Xsql {
  ...
  constructor(sqlConfig, pool) {
    ...
    this.schemaRepository = new SchemaRepository(this.connectionManager, sqlConfig.database, sqlConfig.databases);

    // Initialize Services (pass connectionManager for consistency)
    this.rlsService = new RlsService(this.connectionManager, sqlConfig.databases);
  ...
  async reloadSchema() {
    return await this.schemaRepository.reloadSchema();
  }

  // Multi-database mode: name resolution (delegates to SchemaRepository)
  isMultiSchema() {
    return this.schemaRepository.isMultiSchema();
  }

  resolveTable(name, profileSchema = null) {
    return this.schemaRepository.resolveTable(name, profileSchema);
  }

  resolveRoutine(name, profileSchema = null) {
    return this.schemaRepository.resolveRoutine(name, profileSchema);
  }
  ...
  // Drain any WHERE EXISTS fragments accumulated by !inner embeds during the
  // last getColumnsForSelectStmt call. Callers (CrudService) must append these
  // to the outer WHERE clause so parent rows with no matching relation get
  // filtered out, matching Supabase/PostgREST !inner semantics.
  getAndClearInnerJoinConditions() {
    return this.queryBuilder.getAndClearInnerJoinConditions();
  }

...
class Xsql {
  ...
  getNestedQuery(parentTable, relationName, selectStr) {
    ...
    let fkToParent = fks ? dataHelp.findObjectInArrayByKey('referenced_table_name', parentTable, fks) : null;
    let fkToChild = parentFks ? dataHelp.findObjectInArrayByKey('referenced_table_name', childTable, parentFks) : null;
    ...
    if (explicitItems.length > 0) {
        for (const item of explicitItems) {
            if (item.type === 'column') {
                 ...
                 cols.push(`'${item.name}'`);
                 cols.push(this.getNestedQuery(tableName, item.name, item.columns));
            }
        }
    }
    ...
    if (explicitItems.length > 0) {
        for (const item of explicitItems) {
             if (item.type === 'column') {
                 ...
                 finalCols.push(this.getNestedQuery(tableName, item.name, item.columns) + ` AS ${item.name}`);
             }
        }
    }

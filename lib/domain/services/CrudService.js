...
class CrudService {
    ...
    if (Array.isArray(data)) {
      ...
      if (returnRepresentation) {
        ...
        if (pks.length === 1 && results.insertId) {
          // Auto-increment PK — use insertId
          whereParts.push('?? = ?');
          selectParams.push(pks[0].column_name, results.insertId);
        } else if (pks.length === 1 && data[pks[0].column_name]) {
          // UUID/string PK provided in the data
          whereParts.push('?? = ?');
          selectParams.push(pks[0].column_name, data[pks[0].column_name]);
        } else if (pks.length > 1) {
          // Composite key — all PK values must be in data
          for (let pk of pks) {
          ...
          // UUID PK generated by trigger — fetch the latest row matching inserted data
          // Use a non-PK unique-ish field to locate the row
          let cols = this.xsql.metaDb.tables[tableName].columns || [];
          for (let col of cols) {
            let val = data[col.column_name];
            if (val !== undefined && val !== null && typeof val !== 'object') {
              whereParts.push('?? = ?');
              selectParams.push(col.column_name, val);
              if (whereParts.length >= 3) break; // 3 fields is enough to be unique
            }
          }
          if (whereParts.length === 0) canFetch = false;
        }

        if (canFetch && whereParts.length > 0) {
          // Order by created_at if available, otherwise just LIMIT 1
          let hasCreatedAt = (this.xsql.metaDb.tables[tableName].columns || [])
            .some(c => c.column_name === 'created_at');
          let orderClause = hasCreatedAt ? ' ORDER BY created_at DESC' : '';
          let selectQuery = 'SELECT * FROM ?? WHERE ' + whereParts.join(' AND ') + orderClause + ' LIMIT 1';
          let rows = await this.xsql.exec(selectQuery, selectParams, context);
          return { results: rows, insertId: results.insertId, affectedRows: results.affectedRows };
        }
      }
  ...
  async list(tableName, queryParams, options = {}, context = null) {
    ...
    // Get columns. This may populate inner-join conditions on the query
    // builder as a side effect when the select string contains `!inner`
    // embeds — drain them immediately after to keep count + main query in sync.
    let cols = this.xsql.getColumnsForSelectStmt(tableName, queryParams);
    const innerJoinConds = this.xsql.getAndClearInnerJoinConditions();
    ...
    // Append `!inner` filters to the outer WHERE so parent rows with no
    // matching relation are excluded. These reference parent columns by
    // table name and must sit at the outer scope, not inside subqueries.
    for (const cond of innerJoinConds) {
      whereObj.query += (whereObj.query ? ' AND ' : ' where ') + cond;
    }
  ...
  async nestedList(parentTable, parentId, childTable, queryParams, options = {}, context = null) {
    ...
    // Append `!inner` filters from nested embeds to the outer WHERE.
    for (const cond of innerJoinConds) {
      whereObj.query += ' AND ' + cond;
    }

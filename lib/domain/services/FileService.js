...
class FileService {
  ...
  /**
   * @param {string} [storageFolder] - Base folder for uploads/downloads (defaults to cwd)
   */
  constructor(storageFolder) {
    this.storageFolder = path.resolve(storageFolder || process.cwd());
  }
   ...
   * Validates file resolves inside the configured storage folder
   *
  ...
  getDownloadPath(filename) {
    ...
    const filePath = path.resolve(this.storageFolder, sanitizedFilename);

    // Defense in depth: never serve anything outside the storage folder
    if (filePath !== this.storageFolder && !filePath.startsWith(this.storageFolder + path.sep)) {

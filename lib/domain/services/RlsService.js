...
class RlsService {
   ...
   * @param {Array<string>|null} [databases] - Databases in multi-database
   *   mode. Each database gets its own _rls_policies table and policy keys
   *   are qualified as `db.table`. Omit for single-database mode.
   */
  constructor(connectionManager, databases = null) {
    this.connectionManager = connectionManager;
    this.multiSchema = Array.isArray(databases) && databases.length > 0;
    this.databases = this.multiSchema ? databases.slice() : null;
    this.rlsPolicies = {}; // Cache for RLS policies
  }

  /**
   * Ensures the RLS policies table exists in the database
   * Creates it if it doesn't exist (one per database in multi-database mode)
   *
   * @returns {Promise<void>}
   */
  async ensureRlsPoliciesTable() {
    const targets = this.multiSchema ? this.databases : [null];

    for (const schema of targets) {
      try {
        const ddl = schema
          ? CONSTANTS.RLS_TABLE.SCHEMA.replace(
              CONSTANTS.RLS_TABLE.NAME,
              `\`${schema}\`.${CONSTANTS.RLS_TABLE.NAME}`
            )
          : CONSTANTS.RLS_TABLE.SCHEMA;
        await this.connectionManager.executeQuery(ddl, []);
        console.log(`RLS policies table ready${schema ? ' (' + schema + ')' : ''}`);
      } catch (err) {
        console.error('Failed to create _rls_policies table' + (schema ? ' in ' + schema : '') + ':', err.message);
   ...
   * In multi-database mode policies are loaded from each database's
   * _rls_policies table and keyed as `db.table`.
   *
   * @returns {Promise<void>}
   */
    ...
    for (const schema of targets) {
      try {
        ...
        const results = await this.connectionManager.executeQuery(
          `SELECT * FROM ${tableRef} WHERE enabled = TRUE`,
          []
        );
        this._indexPolicies(results, schema);
      } catch (err) {
        console.error('Failed to load RLS policies' + (schema ? ' from ' + schema : '') + ':', err.message);
      }
    }
  ...
  /**
   * Indexes policy rows into the in-memory cache
   *
   * @private
   * @param {Array} results - Policy rows
   * @param {string|null} schema - Owning database in multi-database mode
   */
  _indexPolicies(results, schema) {
    for (const policy of results) {
      const tableName = schema ? `${schema}.${policy.table_name}` : policy.table_name;

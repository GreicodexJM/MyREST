...
class QueryBuilderService {
  ...
  constructor(metaDb) {
    this.metaDb = metaDb;
    // Accumulates WHERE EXISTS conditions for !inner join modifiers.
    // Populated during resolveSelectColumns / getColumnsForSelectStmt,
    // must be consumed (and cleared) by the caller after column resolution.
    this._innerJoinConditions = [];
  }

  /**
   * Returns and clears any accumulated inner-join WHERE EXISTS conditions.
   * Call this immediately after getColumnsForSelectStmt() to retrieve
   * conditions that must be appended to the WHERE clause.
   *
   * @returns {string[]} Array of WHERE EXISTS SQL fragments
   */
  getAndClearInnerJoinConditions() {
    const conditions = this._innerJoinConditions;
    this._innerJoinConditions = [];
    return conditions;
  }

  /**
   * Qualifies a related (embedded) resource name with the context table's
   * database in multi-database mode. Embedded resources always live in the
   * same database as the table they are embedded from (foreign keys are
   * captured same-database only).
   *
   * Single-database mode (unqualified context) returns the name unchanged.
   *
   * @param {string} contextTable - Table providing the schema context (metaDb key)
   * @param {string} name - Related resource name from the select string
   * @returns {string} metaDb key for the related resource
   */
  qualifyRelated(contextTable, name) {
    if (!name || name.includes('.')) {
      return name;
    }
    const idx = contextTable ? contextTable.indexOf('.') : -1;
    if (idx === -1) {
      return name;
    }
    return contextTable.slice(0, idx) + '.' + name;
  }
    ...
    if (explicitItems.length > 0) {
      for (const item of explicitItems) {
        if (item.type === 'column') {
          ...
          finalCols.push(this.getNestedQuery(tableName, item.name, item.columns, item.hint, item.joinType) + ` AS ${item.name}`);
        }
      }
    }
    ...
    if (explicitItems.length > 0) {
      for (const item of explicitItems) {
        if (item.type === 'column') {
          ...
          cols.push(`'${item.name}'`);
          cols.push(this.getNestedQuery(tableName, item.name, item.columns, item.hint, item.joinType));
        }
      }
    }
   ...
   * @param {string} joinType - Optional join type modifier ('left' or 'inner')
   * @returns {string} Nested SELECT query
   */
  getNestedQuery(parentTable, relationName, selectStr, hint, joinType) {
    // Embedded resources resolve within the parent's database (multi-db mode)
    let childTable = this.qualifyRelated(parentTable, relationName);
    ...
    if (fkToParent) {
      ...
      let fkCondition = `${childTable}.${fkToParent.column_name} = ${parentTable}.${pk}`;

      // For !inner, add WHERE EXISTS to filter out parent rows with no matching children
      if (joinType === 'inner') {
        this._innerJoinConditions.push(
          `EXISTS (SELECT 1 FROM ${childTable} WHERE ${fkCondition})`
        );
      }

      return `(SELECT CAST(COALESCE(JSON_ARRAYAGG(JSON_OBJECT(${cols})), '[]') AS JSON) FROM ${childTable} WHERE ${fkCondition})`;
    } else if (fkToChild) {
      // N:1 Relationship (parent has FK to child)
      let cols = this.resolveSelectColumnsForJson(childTable, selectStr);
      let pk = this.metaDb.tables[childTable].primaryKeys[0].column_name;
      let fkCondition = `${childTable}.${pk} = ${parentTable}.${fkToChild.column_name}`;

      // For !inner, add WHERE EXISTS to filter out parent rows where FK points to nothing
      if (joinType === 'inner') {
        this._innerJoinConditions.push(
          `EXISTS (SELECT 1 FROM ${childTable} WHERE ${fkCondition})`
        );
      ...
      return `(SELECT JSON_OBJECT(${cols}) FROM ${childTable} WHERE ${fkCondition})`;
    }
